...
if ( ! class_exists( 'GlottypomoTools' ) ):
class GlottypomoTools {
	/**
	 ...
	 */
	static function get_plural_definition( $locale ) {
	...
	/**
	 *	Parse plural definition for UI display
	 *
	 *	@param $definition string
	 *	@return object (
	 *		'n_plurals' 			int number of plurals
	 *		'expression'			string expression from po header
	 *		'plural_definitions'	array
	...
	static function parse_plural_definition( $definition ) {
		...
		$is_condition = true;
		$is_result = false;
		$current_result = $current_condition = '';

		$len = strlen($expr);
		for ( $i=0; $i < $len ; $i++ ) {
			$char = $expr[$i];
			switch ( $char ) {
				case ':': // begin condition. end result.
					$parsed[  ] = self::_format_condition($current_condition);
					$current_condition = '';
					$is_condition = true;
					$is_result = false;
					break;
				case '?': // begin result & end condition
					$is_condition = false;
					$is_result = true;
					$current_result = '';
					break;
		...
		if ( empty($parsed) ) {
			if ( $nplurals > 1 )
				$parsed[1] = self::_format_condition($current_condition);
			else 
				$parsed[0] = '0';
		}
		foreach ( $parsed as $result => $condition ) {
			$retrun->plural_definitions[$result] = array(
				'condition' => $condition,
				'condition_human' => self::make_human_readable_condition($condition),
			);
		}
		for ( $i=0; $i<$nplurals;$i++) {
			if ( ! isset($retrun->plural_definitions[$i] ) )
				$retrun->plural_definitions[$i] = array(
					'condition' => '',
					'condition_human' => __('Everything else','wp-glottypomo'),
				);
		}
		return $retrun;
	}
	 ...
	 *	@return string with outer parenthesises removed
	 */
	private static function _format_condition( $condition ) {
		$condition = trim( $condition ,' ()'); // strip clutters
		$condition .= str_repeat( ')' , substr_count($condition,'(') ); // balance parantheses
		return $condition;
	}
	 ...
	 *
	 *	@param $condition string
	 *	@return string The condition in natural language
	 */
	static function make_human_readable_condition( $condition ) {
		if ( $condition === '0' )
			return __('Always','wp-glottypomo');
		$map_operators = array(
			'n' => __('Number','wp-glottypomo'),
			'%' => __('modulo','wp-glottypomo'),
			'&&' => __('AND','wp-glottypomo'),
			'||' => __('OR','wp-glottypomo'),
			'==' => __('is','wp-glottypomo'),
			'!=' => __('is not','wp-glottypomo'),
			'<=' => __('is less or equal than','wp-glottypomo'),
			'>=' => __('is greater or equal than','wp-glottypomo'),
			'<' => __('is less than','wp-glottypomo'),
			'>' => __('is greater than','wp-glottypomo'),
		);
		$condition = preg_replace('@(\S)([%<>=!]+)(\S)@',"$1 $2 $3",$condition); // readability
		$condition = str_replace( 
			array_keys( $map_operators ) , 
			array_values( $map_operators ) , 
		$condition );
		return $condition;
	}
}
